...
// ** Function to Grab Card from Array
// const getCard = () => {
//     let randomIndex = Math.ceil(Math.random() * 52)
//     let randomCard = CARDS[randomIndex]
//     let cardDisplay = document.createElement('img')
//     cardDisplay.src = randomCard['image']
//     cardDisplay.classList.add('.card')
//     document.getElementById('deck-of-cards').appendChild(cardDisplay)
...
const getCard = () => {
    let randomIndex = Math.ceil(Math.random() * 52)
    let randomCard = CARDS[randomIndex]
    // Here Attempting to Create Parent Table
    let cardDisplayTable = document.createElement('table')
    // Here appending two rows to table created above
    cardDisplayTable.insertAdjacentHTML('beforeend', `<tr id="image-row"><td><img src=${randomCard['image']}></td></tr><tr id="point-row"><td><p>${randomCard['value']}</p></td></tr>`)
    // Here creating image element for table row
    // let cardDisplayImg = document.createElement('img')
    // cardDisplayImg.src = randomCard['image']
    // cardDisplayImg.classList.add('.card')



    // document.querySelector('tbody').appendChild(cardDisplayImg)
    document.getElementById('deck-of-cards').appendChild(cardDisplayTable)
    // console.log(randomCard['image'])
    // console.log(cardDisplayImg)
    console.log(cardDisplayTable)
    // console.log(randomIndex)
    // console.log(randomCard)
}
...
const drawOneBtn = document.getElementById('draw-card')
...
const drawFiveBtn = document.getElementById('pick-five')
drawFiveBtn.addEventListener('click', drawFive)

// ** Reset Function
const reset = () => {
    let cardDiv = document.getElementById('deck-of-cards')
    while (cardDiv.childNodes.length > 0) {
        cardDiv.childNodes[0].remove()
    }
}
